...
This week builds on top of the Lab 2-1 topology; East-Core-Switch-01 is still the only routing device; West-Core-Switch-01 still just trunks traffic through.
...
3. Configured the switch port as an access port on VLAN 1:
```
enable
configure terminal
interface FastEthernet 0/3
 ...
 exit
```
4. Assign the router-side IP to the VLAN 1 interface:
```
interface vlan 1
...
2. **Rogue DHCP Servers / DHCP Spoofing** — an attacker's unauthorized DHCP server races the legitimate one to respond to client DISCOVER broadcasts, then hands out a malicious gateway/DNS to enable man-in-the-middle attacks. Defended through DHCP snooping's trusted/untrusted port model, dynamic ARP inspection, and active scanning for unauthorized servers.
...
> 📝 **Observation:** The DHCP server, default gateway, and DNS server are all the same device (10.0.17.2) on this network, a common setup where a single router/firewall appliance handles all three roles.
